...
public:
    ...
    void redraw(const ChessState &state, const std::vector<std::pair<std::string, std::string>> &command_descs);
...
private:
    ...
    void draw_game_info(const ChessState &state, const std::vector<std::pair<std::string, std::string>> &command_descs);

    static std::string construct_game_info(const ChessState &state, const std::vector<std::pair<std::string, std::string>> &command_descs);

    char &get(int y, int x);

private:
    static const int BOARD_HEIGHT = PieceTextures::TEXTURE_HEIGHT * Chessboard::Height();
    static const int BOARD_WIDTH = PieceTextures::TEXTURE_WIDTH * Chessboard::Width();
    ...
    static const int COLUMN_INDICES_HEIGHT_OFFSET = BOARD_HEIGHT_OFFSET + BOARD_HEIGHT;
    ...
    static const int MESSAGE_BOARD_WIDTH_OFFSET = ROW_INDICES_WIDTH_OFFSET + ROW_INDICES_WIDTH;

    static const int SCREEN_HEIGHT = BOARD_HEIGHT_OFFSET + std::max(BOARD_HEIGHT + COLUMN_INDICES_HEIGHT, MESSAGE_BOARD_HEIGHT);
    static const int SCREEN_WIDTH = BOARD_WIDTH_OFFSET + BOARD_WIDTH + ROW_INDICES_WIDTH + MESSAGE_BOARD_WIDTH + 1;
