...
/*static*/ bool CommandParser::parse(const std::string_view &command_str, std::string &out_command_name, std::vector<std::string> &out_str_params)
{
    ...
    if (!std::regex_match(command_str.begin(), command_str.end(), command_regex))
        return true;
    ...
    std::regex_search(command_str.begin(), command_str.end(), m, command_name_regex);
    out_command_name = m.str();

    // Extract params.
    std::string_view param_str = m.suffix().str();
    while (std::regex_search(param_str.begin(), param_str.end(), m, parameter_regex))
    {
